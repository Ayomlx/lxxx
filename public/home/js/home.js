	...
	b.fn.ronbongpage = function(c) {
		...
		function sa(a) {
			...
			if (!W(a.target)) {
				...
				m || t || (d = X(d), u = d.y, A = d.x, a.find(".fp-slides").length && Math.abs(B - A) > Math.abs(v - u) ? Math.abs(
					...
					(B > A ? l.right && b.fn.ronbongpage.moveSlideRight() : l.left && b.fn.ronbongpage.moveSlideLeft()) : c.autoScrolling &&
		...
		function ha() {
			if (c.responsive) {
				...
				b(window).width() < c.responsive ? a || (b.fn.ronbongpage.setAutoScrolling(!1, "internal"), b("#fp-nav").hide(),
					g.addClass("fp-responsive")) : a && (b.fn.ronbongpage.setAutoScrolling(N.autoScrolling, "internal"), b(
					"#fp-nav").show(), g.removeClass("fp-responsive"))
			}
		}
		...
		b.fn.ronbongpage.setAllowScrolling = function(a, c) {
			if ("undefined" != typeof c) c = c.replace(" ", "").split(","), b.each(c, function(c, d) {
				...
				if (b.fn.ronbongpage.setMouseWheelScrolling(!0), C || P) MSPointer = oa(), b(document).off("touchstart " +
					MSPointer.down).on("touchstart " + MSPointer.down, ta), b(document).off("touchmove " +
					MSPointer.move).on("touchmove " + MSPointer.move, sa)
			} else if (b.fn.ronbongpage.setMouseWheelScrolling(!1), C || P) MSPointer = oa(), b(document).off("touchstart " +
		...
		b(this).length ? (g.css({
			...
			"Error! ronbongpage.js needs to be initialized with a selector. For example: $('#myContainer').ronbongpage();");
	...
	window.Rbslider = function(container, params) {
		...
		s.preloadImages = function() {
			...
			for (var i = 0; i < s.imagesToLoad.length; i++) {
				s.loadImage(s.imagesToLoad[i], (s.imagesToLoad[i].currentSrc || s.imagesToLoad[i].getAttribute('src')), true,
					_onReady)
			}
		};
		...
		s.updateSlidesProgress = function(translate) {
			...
			for (var i = 0; i < s.slides.length; i++) {
				...
				var slideProgress = (offsetCenter - slide.rbsliderSlideOffset - slideCenterOffset) / (slide.rbsliderSlideSize +
					s.params.spaceBetween);
				if (s.params.watchSlidesVisibility) {
					var slideBefore = -(offsetCenter - slide.rbsliderSlideOffset - slideCenterOffset);
					var slideAfter = slideBefore + s.slidesSizesGrid[i];
					var isVisible = (slideBefore >= 0 && slideBefore < s.size) || (slideAfter > 0 && slideAfter <= s.size) || (
		...
		s.updateActiveIndex = function() {
			...
			for (i = 0; i < s.slidesGrid.length; i++) {
				if (typeof s.slidesGrid[i + 1] !== 'undefined') {
					if (translate >= s.slidesGrid[i] && translate < s.slidesGrid[i + 1] - (s.slidesGrid[i + 1] - s.slidesGrid[i]) /
						2) {
						newActiveIndex = i
		...
		s.updatePagination = function() {
			...
			if (s.paginationContainer && s.paginationContainer.length > 0) {
				var bulletsHTML = '';
				var numberOfBullets = s.params.loop ? Math.ceil((s.slides.length - s.loopedSlides * 2) / s.params.slidesPerGroup) :
					s.snapGrid.length;
		...
		s.initEvents = function(detach) {
			...
			if (s.params.preventClicks || s.params.preventClicksPropagation) touchEventsTarget[action]('click', s.preventClicks,
				true)
		};
			...
			if (s.params.slideToClickedSlide && s.clickedIndex !== undefined && s.clickedIndex !== s.activeIndex) {
				...
				if (s.params.loop) {
					...
					if (slideToIndex > s.slides.length - s.params.slidesPerView) {
						s.fixLoop();
						slideToIndex = s.wrapper.children('.' + s.params.slideClass + '[data-rbslider-slide-index="' + realIndex +
							'"]').eq(0).index();
						setTimeout(function() {
						...
						s.fixLoop();
						var duplicatedSlides = s.wrapper.children('.' + s.params.slideClass + '[data-rbslider-slide-index="' +
							realIndex + '"]');
		...
		s.onTouchMove = function(e) {
			...
			if (typeof isScrolling === 'undefined') {
				var touchAngle = Math.atan2(Math.abs(s.touches.currentY - s.touches.startY), Math.abs(s.touches.currentX - s.touches
					.startX)) * 180 / Math.PI;
			...
			if ((diff > 0 && currentTranslate > s.minTranslate())) {
				...
				if (s.params.resistance) currentTranslate = s.minTranslate() - 1 + Math.pow(-s.minTranslate() + startTranslate +
					diff, s.params.resistanceRatio)
			} else if (diff < 0 && currentTranslate < s.maxTranslate()) {
				disableParentRbslider = false;
				if (s.params.resistance) currentTranslate = s.maxTranslate() + 1 - Math.pow(s.maxTranslate() - startTranslate -
					diff, s.params.resistanceRatio)
			}
		...
		s.fixLoop = function() {
			...
			} else if ((s.params.slidesPerView === 'auto' && s.activeIndex >= s.loopedSlides * 2) || (s.activeIndex > s.slides
					.length - s.params.slidesPerView * 2)) {
			...
			fade: {
				...
				setTranslate: function() {
					for (var i = 0; i < s.slides.length; i++) {
						...
						var slideOpacity = s.params.fade.crossFade ? Math.max(1 - Math.abs(slide[0].progress), 0) : 1 + Math.min(Math
							.max(slide[0].progress, -1), 0);
				...
				setTransition: function(duration) {
					...
					if (s.params.virtualTranslate && duration !== 0) {
						...
						s.slides.eq(fadeIndex).transitionEnd(function() {
							var triggerEvents = ['webkitTransitionEnd', 'transitionend', 'oTransitionEnd', 'MSTransitionEnd',
								'msTransitionEnd'
							];
			...
			cube: {
				setTranslate: function() {
					...
					for (var i = 0; i < s.slides.length; i++) {
						...
						var transform = 'rotateX(' + (isH() ? 0 : -slideAngle) + 'deg) rotateY(' + (isH() ? slideAngle : 0) +
						...
						if (s.params.cube.slideShadows) {
							var shadowBefore = isH() ? slide.find('.rbslider-slide-shadow-left') : slide.find(
								'.rbslider-slide-shadow-top');
							var shadowAfter = isH() ? slide.find('.rbslider-slide-shadow-right') : slide.find(
								'.rbslider-slide-shadow-bottom');
					...
					if (s.params.cube.shadow) {
						if (isH()) {
							cubeShadow.transform('translate3d(0px, ' + (s.width / 2 + s.params.cube.shadowOffset) + 'px, ' + (-s.width /
								2) + 'px) rotateX(90deg) rotateZ(0deg) scale(' + (s.params.cube.shadowScale) + ')')
						} else {
							var shadowAngle = Math.abs(wrapperRotate) - Math.floor(Math.abs(wrapperRotate) / 90) * 90;
							var multiplier = 1.5 - (Math.sin(shadowAngle * 2 * Math.PI / 360) / 2 + Math.cos(shadowAngle * 2 * Math.PI /
								360) / 2);
							var scale1 = s.params.cube.shadowScale,
								scale2 = s.params.cube.shadowScale / multiplier,
								offset = s.params.cube.shadowOffset;
							cubeShadow.transform('scale3d(' + scale1 + ', 1, ' + scale2 + ') translate3d(0px, ' + (s.height / 2 + offset) +
								'px, ' + (-s.height / 2 / scale2) + 'px) rotateX(-90deg)')
						}
					}
					var zFactor = (s.isSafari || s.isUiWebView) ? (-s.size / 2) : 0;
					s.wrapper.transform('translate3d(0px,0,' + zFactor + 'px) rotateX(' + (isH() ? 0 : wrapperRotate) +
						'deg) rotateY(' + (isH() ? -wrapperRotate : 0) + 'deg)')
				},
				setTransition: function(duration) {
					s.slides.transition(duration).find(
						'.rbslider-slide-shadow-top, .rbslider-slide-shadow-right, .rbslider-slide-shadow-bottom, .rbslider-slide-shadow-left'
					).transition(duration);
		...
		function handleKeyboard(e) {
			...
			if (document.activeElement && document.activeElement.nodeName && (document.activeElement.nodeName.toLowerCase() ===
					'input' || document.activeElement.nodeName.toLowerCase() === 'textarea')) {
				return
			}
			if (kc === 37 || kc === 39 || kc === 38 || kc === 40) {
				var inView = false;
				if (s.container.parents('.rbslider-slide').length > 0 && s.container.parents('.rbslider-slide-active').length ===
					0) {
					return
				}
				var windowScroll = {
				...
				for (var i = 0; i < rbsliderCoord.length; i++) {
					...
					if (point[0] >= windowScroll.left && point[0] <= windowScroll.left + windowWidth && point[1] >= windowScroll.top &&
		...
		s.parallax = {
			setTranslate: function() {
				s.container.children('[data-rbslider-parallax], [data-rbslider-parallax-x], [data-rbslider-parallax-y]').each(
					function() {
			...
			setTransition: function(duration) {
				...
				s.container.find('[data-rbslider-parallax], [data-rbslider-parallax-x], [data-rbslider-parallax-y]').each(
					function() {
						var el = $(this);
		...
		s.cleanupStyles = function() {
			...
			if (s.slides && s.slides.length) {
				s.slides.removeClass([s.params.slideVisibleClass, s.params.slideActiveClass, s.params.slideNextClass, s.params.slidePrevClass]
					.join(' ')).removeAttr('style').removeAttr('data-rbslider-column').removeAttr('data-rbslider-row')
			}
	...
	Rbslider.prototype = {
		...
		support: {
			...
			transforms3d: (window.Modernizr && Modernizr.csstransforms3d === true) || (function() {
				...
				return ('webkitPerspective' in div || 'MozPerspective' in div || 'OPerspective' in div || 'MsPerspective' in
					div || 'perspective' in div)
			})(),
			flexbox: (function() {
				var div = document.createElement('div').style;
				var styles = (
	...
	var Dom7 = (function() {
		...
		Dom7.prototype = {
			...
			data: function(key, value) {
				if (typeof value === 'undefined') {
					if (this[0]) {
						...
						else if (this[0].dom7ElementDataStorage && (key in this[0].dom7ElementDataStorage)) return this[0].dom7ElementDataStorage[
							key];
						else return undefined
					} else return undefined
				} else {
			...
			outerWidth: function(includeMargins) {
				if (this.length > 0) {
					if (includeMargins) return this[0].offsetWidth + parseFloat(this.css('margin-right')) + parseFloat(this.css(
						'margin-left'));
			...
			outerHeight: function(includeMargins) {
				if (this.length > 0) {
					if (includeMargins) return this[0].offsetHeight + parseFloat(this.css('margin-top')) + parseFloat(this.css(
						'margin-bottom'));
			...
			prev: function(selector) {
				if (this.length > 0) {
					if (selector) {
						if (this[0].previousElementSibling && $(this[0].previousElementSibling).is(selector)) return new Dom7([this[
...
function initCaseList() {
	var b = $(window).width();
	860 > b && 0 == $(".rbslider-item-list-rbslider").size() ? ($(".case-list,.equipment-list").addClass(
		"rbslider-item-list-rbslider").addClass("rbslider-container").find("ul").addClass("rbslider-wrapper").find("li").addClass(
		"rbslider-slide"), rbsliderInterval = setInterval(function() {
		var a = $(".case-list .rbslider-slide").width();
		a > .88 * a && (clearInterval(rbsliderInterval), $(".rbslider-item-list-container .navigation").show(), rbslider2 =
			new Rbslider(".case-list", {
	...
	}, 300)) : 860 < b && 0 < $(".rbslider-item-list-rbslider").size() && (clearInterval(rbsliderInterval), rbslider2 &&
		rbslider2.destroy(), clearInterval(rbsliderInterval2), rbslider3 && rbslider3.destroy(), $(
			".rbslider-item-list-container .navigation").hide(), $(".case-list,.equipment-list").removeClass(
			"rbslider-container").removeClass("rbslider-item-list-rbslider"), $(".case-list").html(caseListHtml), $(
