						...
						<form class="form-horizontal" id="validation-form" novalidate="novalidate">
							<div class="form-group">
								...
								<div class="col-xs-12 col-sm-9">
									<div class="clearfix">
										<input type="text" name="username" id="username" class="col-xs-12 col-sm-3" value="<?= !empty($info['username']) ? $info['username'] : ''; ?>">
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 权限组 </label>

								<div class="col-xs-12 col-sm-9">
									<select class="chosen-select form-control" id="form-field-select-3" name="group_id" style="display: none;">
										<option value="">-请选择-</option>
										<?php foreach($auth_list as $key=>$val){ ?>
											<option value="<?= $val['id']; ?>" <?= !empty($info['group_id']) && $info['group_id'] == $val['id'] ? 'selected' : ''; ?>><?= $val['name']; ?></option>
										<?php }?>
									</select>
								</div>
							</div>
		...
		submitHandler: function (form) {
			...
			.done(function(r) {
	            ...
	            if(re.code == 0){
	                ...
	                setTimeout('window.location.href="/Admin/Account"',1500);
	            }
	        })
		},
	});
